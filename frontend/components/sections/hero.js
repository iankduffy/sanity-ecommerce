...
const HomeSplashScreen = ({heading = '', backgroundImage, tagline ='', id = ''}) => {
  ...
  <section id={id} className={styles.hero} style={{backgroundImage: ``}}>
    <ProgressImage alt={heading} previewUrl={urlFor(backgroundImage).width(500).quality(40).blur(200).auto('format').url()} imageUrl={urlFor(backgroundImage).width(1400).auto('format').url()} srcSetImages={srcSet(backgroundImage, 1800)}/>
