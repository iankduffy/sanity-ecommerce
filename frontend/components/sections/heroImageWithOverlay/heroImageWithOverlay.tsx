...
const heroImageWithOverlay = ({blockContent = '', image, index}: heroImageProps) => {
  ...
  return(
    <section className={styles.heroImageContainer}>
      <picture> 
        <source width={getImageDimensions(image).width.toString()} height={getImageDimensions(image).height.toString()} src={urlFor(image).width(1140).height(300).fit('fill').quality(50).auto('format').url()} srcSet={srcSet(image, 1800, 700)} media="(min-width: 738px)" />
